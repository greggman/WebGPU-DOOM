...
var ML_REJECT = 9;
var ML_BLOCKMAP = 10;
var MAPBLOCKUNITS = 128;
var MAPBLOCKSIZE = MAPBLOCKUNITS * 65536;
var MAPBLOCKSHIFT = 16 + 7;
var MAPBTOFRAC = MAPBLOCKSHIFT - 16;
...
function loadMap(wad2, name) {
  ...
  for (let i = 0; i < lineDefs.length; i++) {
    const ld = lineDefs[i];
    const front = ld.sideNum[0] >= 0 ? sideDefs2[ld.sideNum[0]] : void 0;
    const back = ld.sideNum[1] >= 0 ? sideDefs2[ld.sideNum[1]] : void 0;
  ...
  return { name, vertexes, sectors, sideDefs: sideDefs2, lineDefs, segs, subSectors, nodes, blockMap, reject };
}
...
var maxAmmo = [200, 50, 300, 50];
var clipAmmo = [10, 4, 20, 1];

// src/p_local.ts
var MAXRADIUS = 32 * FRACUNIT;
var ONFLOORZ = -2147483648;
var ONCEILINGZ = 2147483647;
var VIEWHEIGHT = 41 * FRACUNIT;
...
var intercepts = Array.from({ length: MAXINTERCEPTS }, () => ({
  frac: 0,
  isALine: false,
  line: null,
  thing: null
...
function P_RunThinkers() {
  ...
  running = false;
  let any = false;
  for (const t of thinkers) if (t.removed) {
    any = true;
    break;
  }
...
// src/s_sound.ts
var backend = null;
function S_StartSound(origin, sfx) {
  backend?.startSound(origin, sfx);
}
function S_StartSoundAt(x, y, sfx) {
...
function P_CallAction(name, mo2) {
...
var strace = { x: 0, y: 0, dx: 0, dy: 0 };
var t2x = 0;
var t2y = 0;
var sightZStart = 0;
var topSlope = 0;
var bottomSlope = 0;
var sightValidCount = 0;
...
function sectorNumOf(mo2) {
  return mo2.sector ? mo2.sector.index : 0;
}
...
var DI_NODIR = 8;
var MAXPLAYERS = 4;
var MELEERANGE = 64 * FRACUNIT;
var MISSILERANGE = 32 * 64 * FRACUNIT;
var xspeed = [FRACUNIT, 47e3, 0, -47e3, -FRACUNIT, -47e3, 0, 47e3];
var yspeed = [0, 47e3, FRACUNIT, 47e3, 0, -47e3, -FRACUNIT, -47e3];
var opposite = [
  DI_WEST,
  DI_SOUTHWEST,
  DI_SOUTH,
...
function P_LookForPlayers(actor, allAround) {
  ...
  for (; ; actor.lastLook = actor.lastLook + 1 & 3) {
    ...
    const player2 = players[actor.lastLook];
    if (player2.health <= 0) continue;
    if (!player2.mo) continue;
    if (!P_CheckSight(actor, player2.mo)) continue;
    if (!allAround) {
      const an = R_PointToAngle2(actor.x, actor.y, player2.mo.x, player2.mo.y) - actor.angle >>> 0;
      if (an > ANG90 && an < ANG270) {
        const dist = P_AproxDistance(player2.mo.x - actor.x, player2.mo.y - actor.y);
        if (dist > MELEERANGE) continue;
      }
    }
    actor.target = player2.mo;
    return true;
  }
}
var soundValidCount = 0;
var soundTargetRef = null;
...
function A_SkelMissile(actor) {
  ...
  const mo2 = env.spawnMissile(actor, actor.target, MT.MT_TRACER);
  actor.z = actor.z - 16 * FRACUNIT | 0;
  mo2.x = mo2.x + mo2.momx | 0;
  mo2.y = mo2.y + mo2.momy | 0;
  mo2.tracer = actor.target;
}
var TRACEANGLE = 201326592;
...
function A_FatAttack1(actor) {
  ...
  const mo2 = env.spawnMissile(actor, actor.target, MT.MT_FATSHOT);
  mo2.angle = mo2.angle + FATSPREAD >>> 0;
  const fine = mo2.angle >>> ANGLETOFINESHIFT & FINEMASK;
  const speed = mobjInfo[mo2.type].speed;
  mo2.momx = FixedMul(speed, finecosine[fine]);
  mo2.momy = FixedMul(speed, finesine[fine]);
}
function A_FatAttack2(actor) {
  ...
  env.spawnMissile(actor, actor.target, MT.MT_FATSHOT);
  const mo2 = env.spawnMissile(actor, actor.target, MT.MT_FATSHOT);
  mo2.angle = mo2.angle - FATSPREAD * 2 >>> 0;
  const fine = mo2.angle >>> ANGLETOFINESHIFT & FINEMASK;
  const speed = mobjInfo[mo2.type].speed;
  mo2.momx = FixedMul(speed, finecosine[fine]);
  mo2.momy = FixedMul(speed, finesine[fine]);
...
function A_FatAttack3(actor) {
  if (!actor.target) return;
  A_FaceTarget(actor);
  let mo2 = env.spawnMissile(actor, actor.target, MT.MT_FATSHOT);
  mo2.angle = mo2.angle - FATSPREAD / 2 >>> 0;
  let fine = mo2.angle >>> ANGLETOFINESHIFT & FINEMASK;
  let speed = mobjInfo[mo2.type].speed;
  mo2.momx = FixedMul(speed, finecosine[fine]);
  mo2.momy = FixedMul(speed, finesine[fine]);
  mo2 = env.spawnMissile(actor, actor.target, MT.MT_FATSHOT);
  mo2.angle = mo2.angle + FATSPREAD / 2 >>> 0;
  fine = mo2.angle >>> ANGLETOFINESHIFT & FINEMASK;
  speed = mobjInfo[mo2.type].speed;
  mo2.momx = FixedMul(speed, finecosine[fine]);
  mo2.momy = FixedMul(speed, finesine[fine]);
}
var SKULLSPEED = 20 * FRACUNIT;
...
function A_PlayerScream(mo2) {
  S_StartSound(mo2, mo2.health < -50 ? "sfx_pdiehi" : "sfx_pldeth");
}
...
function A_BossDeath(mo2) {
  env.bossDeath?.(mo2);
}
function A_KeenDie(mo2) {
  A_Fall(mo2);
  env.bossDeath?.(mo2);
}
...
// src/p_mobj.ts
var spawnLog = null;
var GRAVITY = FRACUNIT;
var MAXMOVE = 30 * FRACUNIT;
var STOPSPEED = 4096;
var FRICTION = 59392;
var S_NULL = 0;
var env2;
...
var thinkerOf = /* @__PURE__ */ new WeakMap();
function P_SetMobjState(mo2, state) {
  do {
    if (state === S_NULL) {
      mo2.state = S_NULL;
      P_RemoveMobj(mo2);
    ...
    mo2.state = state;
    mo2.tics = st.tics;
    mo2.sprite = st.sprite;
    mo2.frame = st.frame;
    if (st.action) {
      P_CallAction(st.action, mo2);
      if (mo2.removed) return false;
    }
    state = st.nextState;
  } while (mo2.tics === 0);
  return true;
...
function P_SpawnMobj(x, y, z, type) {
  ...
  mo2.lastLook = P_Random() % MAXPLAYERS;
  spawnLog?.(type, x, y, pRandomCount());
  P_SetThingPosition(mo2);
  const sec = env2.sectorAt(x, y);
  mo2.floorZ = sec.floorHeight;
  mo2.ceilingZ = sec.ceilingHeight;
  if (z === ONFLOORZ) mo2.z = mo2.floorZ;
  else if (z === ONCEILINGZ) mo2.z = mo2.ceilingZ - info.height | 0;
  else mo2.z = z;
  const thinker = { removed: false, tick: () => P_MobjThinker(mo2) };
  thinkerOf.set(mo2, thinker);
  P_AddThinker(thinker);
  return mo2;
}
function P_RemoveMobj(mo2) {
  P_UnsetThingPosition(mo2);
  mo2.removed = true;
  const t = thinkerOf.get(mo2);
  if (t) P_RemoveThinker(t);
}
function P_XYMovement(mo2) {
  if (mo2.momx === 0 && mo2.momy === 0) return;
  const player2 = mo2.player;
  if (mo2.momx > MAXMOVE) mo2.momx = MAXMOVE;
  else if (mo2.momx < -MAXMOVE) mo2.momx = -MAXMOVE;
  if (mo2.momy > MAXMOVE) mo2.momy = MAXMOVE;
  else if (mo2.momy < -MAXMOVE) mo2.momy = -MAXMOVE;
  let xmove = mo2.momx;
  let ymove = mo2.momy;
  do {
    let ptryx;
    ...
    if (xmove > MAXMOVE / 2 || ymove > MAXMOVE / 2) {
      ptryx = mo2.x + (xmove / 2 | 0) | 0;
      ptryy = mo2.y + (ymove / 2 | 0) | 0;
      xmove >>= 1;
      ymove >>= 1;
    } else {
      ptryx = mo2.x + xmove | 0;
    ...
    if (!env2.tryMove(mo2, ptryx, ptryy)) {
      if (mo2.player) {
        env2.slideMove(mo2);
      } else if (mo2.flags & MF.MF_MISSILE) {
        env2.explodeMissile(mo2);
        return;
      } else {
        mo2.momx = 0;
        mo2.momy = 0;
  ...
  if (mo2.flags & (MF.MF_MISSILE | MF.MF_NOGRAVITY)) return;
  if (mo2.z > mo2.floorZ) return;
  if (mo2.flags & MF.MF_CORPSE) {
    if (mo2.momx > FRACUNIT / 4 || mo2.momx < -FRACUNIT / 4 || mo2.momy > FRACUNIT / 4 || mo2.momy < -FRACUNIT / 4) {
      if (mo2.floorZ !== env2.sectorAt(mo2.x, mo2.y).floorHeight) return;
    }
  }
  if (mo2.momx > -STOPSPEED && mo2.momx < STOPSPEED && mo2.momy > -STOPSPEED && mo2.momy < STOPSPEED && (!player2 || player2.cmd.forwardMove === 0 && player2.cmd.sideMove === 0)) {
    ...
    mo2.momx = FixedMul(mo2.momx, FRICTION);
    mo2.momy = FixedMul(mo2.momy, FRICTION);
  }
}
function P_ZMovement(mo2) {
  if (mo2.player && mo2.z < mo2.floorZ) {
    mo2.player.viewHeight -= mo2.floorZ - mo2.z | 0;
    mo2.player.deltaViewHeight = VIEWHEIGHT - mo2.player.viewHeight >> 3;
  }
  mo2.z = mo2.z + mo2.momz | 0;
  if (mo2.z <= mo2.floorZ) {
    if (mo2.momz < 0) {
    ...
    mo2.z = mo2.floorZ;
    if (mo2.flags & MF.MF_MISSILE && !(mo2.flags & MF.MF_NOCLIP)) {
      env2.explodeMissile(mo2);
      return;
    }
  } else if (!(mo2.flags & MF.MF_NOGRAVITY)) {
    if (mo2.momz === 0) mo2.momz = -GRAVITY * 2;
    else mo2.momz = mo2.momz - GRAVITY | 0;
  }
  if (mo2.z + mo2.height > mo2.ceilingZ) {
    if (mo2.momz > 0) mo2.momz = 0;
    mo2.z = mo2.ceilingZ - mo2.height | 0;
    if (mo2.flags & MF.MF_MISSILE && !(mo2.flags & MF.MF_NOCLIP)) {
      env2.explodeMissile(mo2);
      return;
    }
...
var thinkLog = null;
function P_MobjThinker(mo2) {
  thinkLog?.(mo2, pRandomCount());
  if (mo2.momx !== 0 || mo2.momy !== 0) {
    P_XYMovement(mo2);
    if (mo2.removed) return;
  }
  if (mo2.z !== mo2.floorZ || mo2.momz !== 0) {
    P_ZMovement(mo2);
    if (mo2.removed) return;
  }
  if (mo2.tics === -1) return;
  mo2.tics--;
  if (mo2.tics <= 0) {
    P_SetMobjState(mo2, states[mo2.state].nextState);
  }
}
function P_ExplodeMissile(mo2) {
  mo2.momx = mo2.momy = mo2.momz = 0;
  P_SetMobjState(mo2, mobjInfo[mo2.type].deathState);
  mo2.tics -= P_Random() & 3;
  if (mo2.tics < 1) mo2.tics = 1;
  mo2.flags &= ~MF.MF_MISSILE;
  if (mobjInfo[mo2.type].deathSound !== "sfx_None") S_StartSound(mo2, mobjInfo[mo2.type].deathSound);
}
...
// src/p_map.ts
var MELEERANGE2 = 64 * FRACUNIT;
var ML_BLOCKING = 1;
var ML_BLOCKMONSTERS = 2;
var MAXSPECIALCROSS = 8;
var tmThing;
var tmFlags = 0;
var tmX = 0;
var tmY = 0;
...
function PIT_CheckThing(thing) {
  ...
  if (tmThing.flags & MF.MF_MISSILE) {
    ...
    if (tmThing.target && (tmThing.target.type === thing.type || tmThing.target.type === MT.MT_KNIGHT && thing.type === MT.MT_BRUISER || tmThing.target.type === MT.MT_BRUISER && thing.type === MT.MT_KNIGHT)) {
...
function P_UseLines(player2) {
  const mo2 = player2.mo;
  if (!mo2) return;
  useThing = mo2;
  const angle = mo2.angle >>> ANGLETOFINESHIFT & FINEMASK;
  const x1 = mo2.x;
  const y1 = mo2.y;
...
function stairStep(mo2) {
  if (!P_TryMove(mo2, mo2.x, mo2.y + mo2.momy | 0)) {
    P_TryMove(mo2, mo2.x + mo2.momx | 0, mo2.y);
  }
}
function P_SlideMove(mo2) {
  slideMo = mo2;
  let hitCount = 0;
  for (; ; ) {
    if (++hitCount === 3) {
    ...
    let leadX, trailX;
    if (mo2.momx > 0) {
      leadX = mo2.x + mo2.radius | 0;
      trailX = mo2.x - mo2.radius | 0;
    } else {
      leadX = mo2.x - mo2.radius | 0;
      trailX = mo2.x + mo2.radius | 0;
    }
    let leadY, trailY;
    if (mo2.momy > 0) {
      leadY = mo2.y + mo2.radius | 0;
      trailY = mo2.y - mo2.radius | 0;
    } else {
      leadY = mo2.y - mo2.radius | 0;
      trailY = mo2.y + mo2.radius | 0;
    }
    ...
    if (bestSlideFrac > 0) {
      const newX = FixedMul(mo2.momx, bestSlideFrac);
      const newY = FixedMul(mo2.momy, bestSlideFrac);
      if (!P_TryMove(mo2, mo2.x + newX | 0, mo2.y + newY | 0)) {
        stairStep(mo2);
        return;
      }
    }
    ...
    tmXMove = FixedMul(mo2.momx, bestSlideFrac);
    tmYMove = FixedMul(mo2.momy, bestSlideFrac);
    if (bestSlideLine) P_HitSlideLine(bestSlideLine);
    mo2.momx = tmXMove;
    mo2.momy = tmYMove;
    if (P_TryMove(mo2, mo2.x + tmXMove | 0, mo2.y + tmYMove | 0)) return;
  }
}
...
function EV_Teleport(line, side, thing) {
  ...
  for (const sec of level3.sectors) {
    ...
    for (let m = sec.thingList; m; m = m.snext) {
      ...
      const srcFog = env3.spawnMobj(oldX, oldY, oldZ, MT.MT_TFOG);
      S_StartSound(srcFog, "sfx_telept");
      const fine = m.angle >>> ANGLETOFINESHIFT & FINEMASK;
      const dstFog = env3.spawnMobj(
...
var PST_DEAD = 1;
var onGround = false;
function P_Thrust(player2, angle, move) {
  const fine = angle >>> ANGLETOFINESHIFT & FINEMASK;
  const mo2 = player2.mo;
  mo2.momx = mo2.momx + FixedMul(move, finecosine[fine]) | 0;
  mo2.momy = mo2.momy + FixedMul(move, finesine[fine]) | 0;
}
function P_MovePlayer(player2) {
  const cmd = player2.cmd;
  const mo2 = player2.mo;
  mo2.angle = mo2.angle + (cmd.angleTurn << 16) >>> 0;
  onGround = mo2.z <= mo2.floorZ;
  if (cmd.forwardMove !== 0 && onGround) {
    P_Thrust(player2, mo2.angle, cmd.forwardMove * 2048);
  }
  if (cmd.sideMove !== 0 && onGround) {
    P_Thrust(player2, mo2.angle - ANG902 >>> 0, cmd.sideMove * 2048);
  }
  if ((cmd.forwardMove !== 0 || cmd.sideMove !== 0) && mo2.state === S.S_PLAY) {
    P_SetMobjState(mo2, S.S_PLAY_RUN1);
  }
}
function P_CalcHeight(player2, levelTime3) {
  const mo2 = player2.mo;
  player2.bob = FixedMul(mo2.momx, mo2.momx) + FixedMul(mo2.momy, mo2.momy) | 0;
  player2.bob >>= 2;
  if (player2.bob > MAXBOB) player2.bob = MAXBOB;
  if (!onGround) {
    player2.viewZ = mo2.z + VIEWHEIGHT | 0;
    if (player2.viewZ > mo2.ceilingZ - 4 * FRACUNIT) {
      player2.viewZ = mo2.ceilingZ - 4 * FRACUNIT | 0;
    }
    player2.viewZ = mo2.z + player2.viewHeight | 0;
    return;
  }
  const angle = FINEANGLES / 20 * levelTime3 & FINEMASK;
  const bob = FixedMul(player2.bob / 2, finesine[angle]);
  if (player2.state === PST_LIVE) {
    player2.viewHeight = player2.viewHeight + player2.deltaViewHeight | 0;
    if (player2.viewHeight > VIEWHEIGHT) {
      player2.viewHeight = VIEWHEIGHT;
      player2.deltaViewHeight = 0;
    }
    if (player2.viewHeight < VIEWHEIGHT / 2) {
      player2.viewHeight = VIEWHEIGHT / 2;
      if (player2.deltaViewHeight <= 0) player2.deltaViewHeight = 1;
    }
    if (player2.deltaViewHeight !== 0) {
      player2.deltaViewHeight = player2.deltaViewHeight + FRACUNIT / 4 | 0;
      if (player2.deltaViewHeight === 0) player2.deltaViewHeight = 1;
    }
  }
  player2.viewZ = mo2.z + player2.viewHeight + bob | 0;
  if (player2.viewZ > mo2.ceilingZ - 4 * FRACUNIT) {
    player2.viewZ = mo2.ceilingZ - 4 * FRACUNIT | 0;
  }
}
...
function P_PlayerThink(player2, levelTime3) {
  ...
  if (player2.cmd.buttons & BT_USE) {
    if (!player2.useDown) {
      P_UseLines(player2);
      player2.useDown = true;
    }
  } else {
...
// src/p_pspr.ts
var LOWERSPEED = FRACUNIT * 6;
var RAISESPEED = FRACUNIT * 6;
var WEAPONBOTTOM = 128 * FRACUNIT;
var WEAPONTOP = 32 * FRACUNIT;
var BFGCELLS = 40;
var ps_weapon = 0;
var ps_flash = 1;
var NUMPSPRITES = 2;
...
var MELEERANGE3 = 64 * FRACUNIT;
var MISSILERANGE2 = 32 * 64 * FRACUNIT;
var bulletSlope = 0;
function P_SetPsprite(player2, position, stnum) {
  const psp = player2.psprites[position];
  do {
    if (!stnum) {
      psp.state = 0;
      break;
    ...
    psp.tics = state.tics;
    if (state.action) {
      callWeaponAction(state.action, player2, psp);
      if (!psp.state) break;
    }
...
function P_BringUpWeapon(player2) {
  ...
  P_SetPsprite(player2, ps_weapon, newState);
}
function P_CheckAmmo(player2) {
  const ammo = weaponInfo[player2.readyWeapon].ammo;
  let count = 1;
  if (player2.readyWeapon === WP.wp_bfg) count = BFGCELLS;
  else if (player2.readyWeapon === WP.wp_supershotgun) count = 2;
  if (ammo === AM.am_noammo || player2.ammo[ammo] >= count) return true;
  do {
    if (player2.weaponOwned[WP.wp_plasma] && player2.ammo[AM.am_cell]) {
      player2.pendingWeapon = WP.wp_plasma;
    } else if (player2.weaponOwned[WP.wp_chaingun] && player2.ammo[AM.am_clip]) {
      player2.pendingWeapon = WP.wp_chaingun;
    } else if (player2.weaponOwned[WP.wp_shotgun] && player2.ammo[AM.am_shell]) {
      player2.pendingWeapon = WP.wp_shotgun;
    } else if (player2.ammo[AM.am_clip]) {
      player2.pendingWeapon = WP.wp_pistol;
    } else if (player2.weaponOwned[WP.wp_chainsaw]) {
      player2.pendingWeapon = WP.wp_chainsaw;
    } else if (player2.weaponOwned[WP.wp_missile] && player2.ammo[AM.am_misl]) {
      player2.pendingWeapon = WP.wp_missile;
    } else if (player2.weaponOwned[WP.wp_bfg] && player2.ammo[AM.am_cell] > 40) {
      player2.pendingWeapon = WP.wp_bfg;
    } else {
      player2.pendingWeapon = WP.wp_fist;
    }
  } while (player2.pendingWeapon === WP_NOCHANGE);
  P_SetPsprite(player2, ps_weapon, weaponInfo[player2.readyWeapon].downState);
  return false;
}
function P_FireWeapon(player2) {
  if (!P_CheckAmmo(player2)) return;
  P_SetMobjState(player2.mo, S.S_PLAY_ATK1);
  P_SetPsprite(player2, ps_weapon, weaponInfo[player2.readyWeapon].atkState);
  env4.noiseAlert(player2.mo, player2.mo);
}
function A_WeaponReady(player2, psp) {
  const mo2 = player2.mo;
  ...
  if (player2.cmd.buttons & 1) {
    if (!player2.attackDown || player2.readyWeapon !== WP.wp_missile && player2.readyWeapon !== WP.wp_bfg) {
      player2.attackDown = true;
      P_FireWeapon(player2);
      return;
    }
  } else {
    player2.attackDown = false;
  }
  let angle = 128 * levelTime() & FINEMASK;
  psp.sx = FRACUNIT + FixedMul(player2.bob, finecosine[angle]) | 0;
  angle &= FINEANGLES / 2 - 1;
  psp.sy = WEAPONTOP + FixedMul(player2.bob, finesine[angle]) | 0;
}
function A_ReFire(player2, _psp) {
  if (player2.cmd.buttons & 1 && player2.pendingWeapon === WP_NOCHANGE && player2.health) {
    player2.refire++;
    P_FireWeapon(player2);
  } else {
    player2.refire = 0;
...
function A_CheckReload(player2, _psp) {
  P_CheckAmmo(player2);
}
function A_Lower(player2, psp) {
  ...
  if (!player2.health) {
    P_SetPsprite(player2, ps_weapon, 0);
    return;
  }
  player2.readyWeapon = player2.pendingWeapon;
  P_BringUpWeapon(player2);
}
function A_Raise(player2, psp) {
  psp.sy = psp.sy - RAISESPEED | 0;
  if (psp.sy > WEAPONTOP) return;
  psp.sy = WEAPONTOP;
  P_SetPsprite(player2, ps_weapon, weaponInfo[player2.readyWeapon].readyState);
}
function A_GunFlash(player2, _psp) {
  P_SetMobjState(player2.mo, S.S_PLAY_ATK2);
  P_SetPsprite(player2, ps_flash, weaponInfo[player2.readyWeapon].flashState);
}
function P_BulletSlope(mo2) {
  let an = mo2.angle;
  bulletSlope = env4.aimLineAttack(mo2, an, 16 * 64 * FRACUNIT);
  if (!env4.lineTarget()) {
    an = an + (1 << 26) >>> 0;
    bulletSlope = env4.aimLineAttack(mo2, an, 16 * 64 * FRACUNIT);
    if (!env4.lineTarget()) {
...
function P_GunShot(mo2, accurate) {
  const damage = 5 * (P_Random() % 3 + 1);
  let angle = mo2.angle;
  if (!accurate) angle = angle + (P_Random() - P_Random() << 18) >>> 0;
  env4.lineAttack(mo2, angle, MISSILERANGE2, bulletSlope, damage);
}
function A_Punch(player2, _psp) {
  let damage = P_Random() % 10 + 1 << 1;
  if (player2.powers[1]) damage *= 10;
  const mo2 = player2.mo;
  const angle = mo2.angle + (P_Random() - P_Random() << 18) >>> 0;
  const slope = env4.aimLineAttack(mo2, angle, MELEERANGE3);
  env4.lineAttack(mo2, angle, MELEERANGE3, slope, damage);
  S_StartSound(mo2, "sfx_punch");
  const t = env4.lineTarget();
  if (t) mo2.angle = R_PointToAngle2(mo2.x, mo2.y, t.x, t.y);
}
function A_FirePistol(player2, _psp) {
  const mo2 = player2.mo;
  S_StartSound(mo2, "sfx_pistol");
  P_SetMobjState(mo2, S.S_PLAY_ATK2);
  player2.ammo[weaponInfo[player2.readyWeapon].ammo]--;
  P_SetPsprite(player2, ps_flash, weaponInfo[player2.readyWeapon].flashState);
  P_BulletSlope(mo2);
  P_GunShot(mo2, !player2.refire);
}
function A_FireShotgun(player2, _psp) {
  const mo2 = player2.mo;
  S_StartSound(mo2, "sfx_shotgn");
  P_SetMobjState(mo2, S.S_PLAY_ATK2);
  player2.ammo[weaponInfo[player2.readyWeapon].ammo]--;
  P_SetPsprite(player2, ps_flash, weaponInfo[player2.readyWeapon].flashState);
  P_BulletSlope(mo2);
  for (let i = 0; i < 7; i++) P_GunShot(mo2, false);
}
function A_FireCGun(player2, psp) {
  const mo2 = player2.mo;
  if (!player2.ammo[weaponInfo[player2.readyWeapon].ammo]) return;
  S_StartSound(mo2, "sfx_pistol");
  P_SetMobjState(mo2, S.S_PLAY_ATK2);
  player2.ammo[weaponInfo[player2.readyWeapon].ammo]--;
  P_SetPsprite(
    player2,
    ps_flash,
    weaponInfo[player2.readyWeapon].flashState + psp.state - S.S_CHAIN1
  );
  P_BulletSlope(mo2);
  P_GunShot(mo2, !player2.refire);
}
function A_Saw(player2, _psp) {
  const damage = 2 * (P_Random() % 10 + 1);
  const mo2 = player2.mo;
  const angle = mo2.angle + (P_Random() - P_Random() << 18) >>> 0;
  const slope = env4.aimLineAttack(mo2, angle, MELEERANGE3 + 1);
  env4.lineAttack(mo2, angle, MELEERANGE3 + 1, slope, damage);
  const t = env4.lineTarget();
  if (!t) {
    S_StartSound(mo2, "sfx_sawful");
    return;
  }
  S_StartSound(mo2, "sfx_sawhit");
  const targetAngle = R_PointToAngle2(mo2.x, mo2.y, t.x, t.y);
  const delta = targetAngle - mo2.angle >>> 0;
  if (delta > ANG180) {
    if ((delta | 0) < -ANG90 / 20) mo2.angle = targetAngle + ANG90 / 21 >>> 0;
    else mo2.angle = mo2.angle - ANG90 / 20 >>> 0;
  } else {
    if (delta > ANG90 / 20) mo2.angle = targetAngle - ANG90 / 21 >>> 0;
    else mo2.angle = mo2.angle + ANG90 / 20 >>> 0;
  }
  mo2.flags |= MF.MF_JUSTATTACKED;
}
function A_FireMissile(player2, _psp) {
  player2.ammo[weaponInfo[player2.readyWeapon].ammo]--;
  env4.spawnPlayerMissile(player2.mo, MT.MT_ROCKET);
}
function A_FirePlasma(player2, _psp) {
  player2.ammo[weaponInfo[player2.readyWeapon].ammo]--;
  P_SetPsprite(player2, ps_flash, weaponInfo[player2.readyWeapon].flashState + (P_Random() & 1));
  env4.spawnPlayerMissile(player2.mo, MT.MT_PLASMA);
}
function A_FireBFG(player2, _psp) {
  player2.ammo[weaponInfo[player2.readyWeapon].ammo] -= BFGCELLS;
  env4.spawnPlayerMissile(player2.mo, MT.MT_BFG);
}
function A_BFGsound(player2, _psp) {
  S_StartSound(player2.mo, "sfx_bfg");
}
function A_BFGSpray(mo2) {
  const source = mo2.target;
  if (!source) return;
  for (let i = 0; i < 40; i++) {
    const an = mo2.angle - ANG90 / 2 + ANG90 / 40 * i >>> 0;
...
function A_FireShotgun2(player2, _psp) {
  const mo2 = player2.mo;
  S_StartSound(mo2, "sfx_dshtgn");
  P_SetMobjState(mo2, S.S_PLAY_ATK2);
  player2.ammo[weaponInfo[player2.readyWeapon].ammo] -= 2;
  P_SetPsprite(player2, ps_flash, weaponInfo[player2.readyWeapon].flashState);
  P_BulletSlope(mo2);
  for (let i = 0; i < 20; i++) {
    const damage = 5 * (P_Random() % 3 + 1);
    const angle = mo2.angle + (P_Random() - P_Random() << 19) >>> 0;
    env4.lineAttack(
      mo2,
      angle,
      MISSILERANGE2,
...
function A_OpenShotgun2(player2, _psp) {
  S_StartSound(player2.mo, "sfx_dbopn");
}
function A_LoadShotgun2(player2, _psp) {
  S_StartSound(player2.mo, "sfx_dbload");
}
function A_CloseShotgun2(player2, psp) {
  S_StartSound(player2.mo, "sfx_dbcls");
  A_ReFire(player2, psp);
}
function A_Light0(player2, _psp) {
  player2.extraLight = 0;
}
function A_Light1(player2, _psp) {
  player2.extraLight = 1;
}
function A_Light2(player2, _psp) {
  player2.extraLight = 2;
}
function P_SetupPsprites(player2) {
  for (let i = 0; i < NUMPSPRITES; i++) player2.psprites[i].state = 0;
  player2.pendingWeapon = player2.readyWeapon;
  P_BringUpWeapon(player2);
}
function P_MovePsprites(player2) {
  for (let i = 0; i < NUMPSPRITES; i++) {
    const psp = player2.psprites[i];
    if (!psp.state) continue;
    if (psp.tics !== -1) {
      psp.tics--;
      if (!psp.tics) P_SetPsprite(player2, i, states[psp.state].nextState);
    }
  }
  player2.psprites[ps_flash].sx = player2.psprites[ps_weapon].sx;
  player2.psprites[ps_flash].sy = player2.psprites[ps_weapon].sy;
}
var weaponActions = /* @__PURE__ */ new Map();
function P_RegisterWeaponActions(table) {
  for (const [name, fn] of Object.entries(table)) weaponActions.set(name, fn);
}
function callWeaponAction(name, player2, psp) {
  weaponActions.get(name)?.(player2, psp);
}
var levelTimeFn = () => 0;
function P_SetPsprLevelTime(fn) {
  levelTimeFn = fn;
}
var levelTime = () => levelTimeFn();
...
function P_FindLowestFloorSurrounding(level10, sec) {
  let floor = sec.floorHeight;
  for (const li of sec.lineIndices) {
    const other = getNextSector(level10.lines[li], sec);
    if (other && other.floorHeight < floor) floor = other.floorHeight;
  }
  return floor;
}
function P_FindHighestFloorSurrounding(level10, sec) {
  let floor = -500 * 65536;
  for (const li of sec.lineIndices) {
    const other = getNextSector(level10.lines[li], sec);
    if (other && other.floorHeight > floor) floor = other.floorHeight;
...
function P_FindNextHighestFloor(level10, sec, currentHeight) {
...
function P_FindLowestCeilingSurrounding(level10, sec) {
...
function P_FindHighestCeilingSurrounding(level10, sec) {
  let height = 0;
  for (const li of sec.lineIndices) {
    const other = getNextSector(level10.lines[li], sec);
    if (other && other.ceilingHeight > height) height = other.ceilingHeight;
...
function P_FindMinSurroundingLight(level10, sec, max) {
  let min = max;
  for (const li of sec.lineIndices) {
    const other = getNextSector(level10.lines[li], sec);
...
// src/p_doors.ts
function doorSound(door, opening) {
  const blaze = door.speed >= VDOORSPEED * 4;
  const sfx = opening ? blaze ? "sfx_bdopn" : "sfx_doropn" : blaze ? "sfx_bdcls" : "sfx_dorcls";
  S_StartSoundAt(door.sector.soundX, door.sector.soundY, sfx);
}
var VDOORSPEED = FRACUNIT * 2;
var VDOORWAIT = 150;
var level5;
...
var platRandom = () => P_Random();

// src/p_ceilng.ts
var CEILSPEED = FRACUNIT;
var level7;
var activeCeilings = [];
...
// src/p_switch.ts
var BUTTONTIME = 35;
var SLOTS = ["topTexture", "midTexture", "bottomTexture"];
var pairs = /* @__PURE__ */ new Map();
var sideDefs = [];
var buttons = [];
var geoDirty = false;
function P_ChangeSwitchTexture(line, useAgain, special) {
...
// src/p_spec.ts
var REPEATABLE_USE = /* @__PURE__ */ new Set([42, 61, 63, 114, 115, 116, 62, 123, 45, 60, 64, 65, 69, 70, 87, 89]);
var env6 = {};
function P_SetSpecEnv(e) {
  env6 = e;
}
function P_PlayerInSpecialSector(player2, levelTime3) {
  const mo2 = player2.mo;
  const sector = mo2.sector;
  if (!sector) return;
  if (mo2.z !== sector.floorHeight) return;
  switch (sector.special) {
    case 5:
      if (!player2.powers[
        ...
        env6.damageMobj?.(mo2, null, null, 10);
      }
      break;
    case 7:
      if (!player2.powers[3] && !(levelTime3 & 31)) {
        env6.damageMobj?.(mo2, null, null, 5);
      }
      break;
    case 16:
    // super hellslime
    case 4:
      if (!player2.powers[3] || P_Random() < 5) {
        if (!(levelTime3 & 31)) env6.damageMobj?.(mo2, null, null, 20);
      }
      break;
    case 9:
      player2.secretCount++;
    ...
    case 11:
      if (!(levelTime3 & 31)) env6.damageMobj?.(mo2, null, null, 20);
      if (player2.health <= 10) env6.exitLevel?.(false);
      break;
    default:
      break;
  }
...
function P_UseSpecialLine(thing, line, side) {
  const orig = line.special;
  if (!thing.player) {
    if (line.flags & 32) return false;
    switch (line.special) {
      case 1:
      // manual door raise
      case 32:
...
function P_Ticker(players) {
  for (const p of players) {
    P_PlayerThink(p, levelTime2);
    if (p.state !== 1) P_PlayerInSpecialSector(p, levelTime2);
  }
  P_RunThinkers();
  P_UpdateButtons();
  levelTime2++;
...
var armorPointsFor = [0, 100, 200];
var PICKUP_MSG = {
  ARM1: "PICKED UP THE ARMOR",
  ARM2: "PICKED UP THE MEGAARMOR",
  BON1: "PICKED UP A HEALTH BONUS",
  BON2: "PICKED UP AN ARMOR BONUS",
  SOUL: "PICKED UP A SOULSPHERE",
  BKEY: "PICKED UP A BLUE KEYCARD",
  YKEY: "PICKED UP A YELLOW KEYCARD",
  RKEY: "PICKED UP A RED KEYCARD",
  BSKU: "PICKED UP A BLUE SKULL KEY",
  YSKU: "PICKED UP A YELLOW SKULL KEY",
  RSKU: "PICKED UP A RED SKULL KEY",
  STIM: "PICKED UP A STIMPACK",
  MEDI: "PICKED UP A MEDIKIT",
  PINV: "PICKED UP INVULNERABILITY",
  PSTR: "PICKED UP BERSERK",
  PINS: "PICKED UP PARTIAL INVISIBILITY",
  SUIT: "PICKED UP A RADIATION SUIT",
  PMAP: "PICKED UP THE AREA MAP",
  PVIS: "PICKED UP LIGHT GOGGLES",
  CLIP: "PICKED UP A CLIP",
  AMMO: "PICKED UP A BOX OF BULLETS",
  ROCK: "PICKED UP A ROCKET",
  BROK: "PICKED UP A BOX OF ROCKETS",
  CELL: "PICKED UP AN ENERGY CELL",
  CELP: "PICKED UP AN ENERGY CELL PACK",
  SHEL: "PICKED UP SOME SHOTGUN SHELLS",
  SBOX: "PICKED UP A BOX OF SHELLS",
  BPAK: "PICKED UP A BACKPACK",
  BFUG: "PICKED UP THE BFG9000",
  MGUN: "PICKED UP THE CHAINGUN",
  CSAW: "PICKED UP THE CHAINSAW",
  LAUN: "PICKED UP THE ROCKET LAUNCHER",
  PLAS: "PICKED UP THE PLASMA GUN",
  SHOT: "PICKED UP THE SHOTGUN",
  SGN2: "PICKED UP THE SUPER SHOTGUN"
};
function P_GiveAmmo(player2, ammo, num) {
  if (ammo === AM.am_noammo) return false;
  if (ammo < 0 || ammo >= maxAmmo.length) return false;
  if (player2.ammo[ammo] === player2.maxAmmo[ammo]) return false;
  let amount = num ? num * clipAmmo[ammo] : clipAmmo[ammo] / 2 | 0;
  const skill = env7.skill();
  if (skill === 0 || skill === 4) amount <<= 1;
  const oldAmmo = player2.ammo[ammo];
  player2.ammo[ammo] += amount;
  if (player2.ammo[ammo] > player2.maxAmmo[ammo]) player2.ammo[ammo] = player2.maxAmmo[ammo];
  if (oldAmmo) return true;
  switch (ammo) {
    case AM.am_clip:
      if (player2.readyWeapon === WP.wp_fist) {
        player2.pendingWeapon = player2.weaponOwned[WP.wp_chaingun] ? WP.wp_chaingun : WP.wp_pistol;
      }
      break;
    case AM.am_shell:
      if (player2.readyWeapon === WP.wp_fist || player2.readyWeapon === WP.wp_pistol) {
        if (player2.weaponOwned[WP.wp_shotgun]) player2.pendingWeapon = WP.wp_shotgun;
      }
      break;
    case AM.am_cell:
      if (player2.readyWeapon === WP.wp_fist || player2.readyWeapon === WP.wp_pistol) {
        if (player2.weaponOwned[WP.wp_plasma]) player2.pendingWeapon = WP.wp_plasma;
      }
      break;
    case AM.am_misl:
      if (player2.readyWeapon === WP.wp_fist) {
        if (player2.weaponOwned[WP.wp_missile]) player2.pendingWeapon = WP.wp_missile;
      }
      break;
    default:
      break;
...
function P_GiveWeapon(player2, weapon, dropped) {
  const ammo = weaponInfo[weapon].ammo;
  let gaveAmmo = false;
  let gaveWeapon = false;
  if (ammo !== AM.am_noammo) {
    gaveAmmo = P_GiveAmmo(player2, ammo, dropped ? 1 : 2);
  }
  if (!player2.weaponOwned[weapon]) {
    gaveWeapon = true;
    player2.weaponOwned[weapon] = true;
    player2.pendingWeapon = weapon;
  }
  return gaveWeapon || gaveAmmo;
}
function P_GiveBody(player2, num) {
  if (player2.health >= 100) return false;
  player2.health += num;
  if (player2.health > 100) player2.health = 100;
  if (player2.mo) player2.mo.health = player2.health;
  return true;
}
function P_GiveArmor(player2, armorType) {
  const hits = armorPointsFor[armorType];
  if (player2.armorPoints >= hits) return false;
  player2.armorType = armorType;
  player2.armorPoints = hits;
  return true;
}
function P_GiveCard(player2, card) {
  if (player2.cards[card]) return;
...
function P_TouchSpecialThing(special, toucher) {
  ...
  switch (spr) {
    // --- armour ---
    case "ARM1":
      if (!P_GiveArmor(player2, 1)) return;
      break;
    case "ARM2":
      if (!P_GiveArmor(player2, 2)) return;
      break;
    // --- bonuses: these go ABOVE 100, which is the whole point of them ---
    case "BON1":
      player2.health++;
      if (player2.health > 200) player2.health = 200;
      if (player2.mo) player2.mo.health = player2.health;
      break;
    case "BON2":
      player2.armorPoints++;
      if (player2.armorPoints > 200) player2.armorPoints = 200;
      if (!player2.armorType) player2.armorType = 1;
      break;
    case "SOUL":
      player2.health += 100;
      if (player2.health > 200) player2.health = 200;
    ...
    case "BKEY":
      P_GiveCard(player2, 0);
      break;
    case "YKEY":
      P_GiveCard(player2, 1);
      break;
    case "RKEY":
      P_GiveCard(player2, 2);
      break;
    case "BSKU":
      P_GiveCard(player2, 3);
      break;
    case "YSKU":
      P_GiveCard(player2, 4);
      break;
    case "RSKU":
      P_GiveCard(player2, 5);
      break;
    // --- health ---
    case "STIM":
      if (!P_GiveBody(player2, 10)) return;
      break;
    case "MEDI":
      if (!P_GiveBody(player2, 25)) return;
      break;
    // --- powerups ---
    case "PINV":
      if (!P_GivePower(player2, 0)) return;
      break;
    case "PSTR":
      if (!P_GivePower(player2, 1)) return;
      if (player2.readyWeapon !== WP.wp_fist) player2.pendingWeapon = WP.wp_fist;
      break;
    case "PINS":
      if (!P_GivePower(player2, 2)) return;
      break;
    case "SUIT":
      if (!P_GivePower(player2, 3)) return;
      break;
    case "PMAP":
      if (!P_GivePower(player2, 4)) return;
      break;
    case "PVIS":
      if (!P_GivePower(player2, 5)) return;
      break;
    // --- ammo ---
    case "CLIP":
      if (!P_GiveAmmo(player2, AM.am_clip, special.flags & MF.MF_DROPPED ? 0 : 1)) return;
      break;
    case "AMMO":
      if (!P_GiveAmmo(player2, AM.am_clip, 5)) return;
      break;
    case "ROCK":
      if (!P_GiveAmmo(player2, AM.am_misl, 1)) return;
      break;
    case "BROK":
      if (!P_GiveAmmo(player2, AM.am_misl, 5)) return;
      break;
    case "CELL":
      if (!P_GiveAmmo(player2, AM.am_cell, 1)) return;
      break;
    case "CELP":
      if (!P_GiveAmmo(player2, AM.am_cell, 5)) return;
      break;
    case "SHEL":
      if (!P_GiveAmmo(player2, AM.am_shell, 1)) return;
      break;
    case "SBOX":
      if (!P_GiveAmmo(player2, AM.am_shell, 5)) return;
      break;
    case "BPAK": {
      if (!player2.backpack) {
        for (let i = 0; i < maxAmmo.length; i++) player2.maxAmmo[i] = maxAmmo[i] * 2;
        player2.backpack = true;
      }
      for (let i = 0; i < maxAmmo.length; i++) P_GiveAmmo(player2, i, 1);
      break;
    }
    // --- weapons ---
    case "BFUG":
      if (!P_GiveWeapon(player2, WP.wp_bfg, false)) return;
      break;
    case "MGUN":
      if (!P_GiveWeapon(player2, WP.wp_chaingun, (special.flags & MF.MF_DROPPED) !== 0)) return;
      break;
    case "CSAW":
      if (!P_GiveWeapon(player2, WP.wp_chainsaw, false)) return;
      break;
    case "LAUN":
      if (!P_GiveWeapon(player2, WP.wp_missile, false)) return;
      break;
    case "PLAS":
      if (!P_GiveWeapon(player2, WP.wp_plasma, false)) return;
      break;
    case "SHOT":
      if (!P_GiveWeapon(player2, WP.wp_shotgun, (special.flags & MF.MF_DROPPED) !== 0)) return;
      break;
    case "SGN2":
      if (!P_GiveWeapon(player2, WP.wp_supershotgun, (special.flags & MF.MF_DROPPED) !== 0)) return;
      break;
    default:
      return;
  }
  if (special.flags & MF.MF_COUNTITEM) player2.itemCount++;
  P_RemoveMobj(special);
  player2.bonusCount += BONUSADD;
  const msg = PICKUP_MSG[spr];
  if (msg) player2.message = msg;
  const sound = ["BFUG", "MGUN", "CSAW", "LAUN", "PLAS", "SHOT", "SGN2"].includes(spr) ? "sfx_wpnup" : ["SOUL", "PINV", "PSTR", "PINS", "SUIT", "PMAP", "PVIS"].includes(spr) ? "sfx_getpow" : "sfx_itemup";
...
function P_DamageMobj(target, inflictor, source, damage) {
  ...
  const player2 = target.player;
  if (player2 && env7.skill() === 0) damage >>= 1;
  ...
  if (player2) {
    if (damage < 1e3 && (player2.cheatGod || player2.powers[
      0
      /* invuln */
    ])) return;
    if (player2.armorType) {
      let saved = player2.armorType === 1 ? Math.floor(damage / 3) : Math.floor(damage / 2);
      if (player2.armorPoints <= saved) {
        saved = player2.armorPoints;
        player2.armorType = 0;
      }
      player2.armorPoints -= saved;
      damage -= saved;
    }
    player2.health -= damage;
    if (player2.health < 0) player2.health = 0;
    player2.attacker = source;
    player2.damageCount += damage;
    if (player2.damageCount > 100) player2.damageCount = 100;
  }
...
function G_LoadLevel(wad2, mapName, skill = 2) {
  ...
  P_SetEnemyEnv({
    ...
    useSpecialLine: (mo3, lineIndex, side) => P_UseSpecialLine(mo3, sim.lines[lineIndex], side),
  ...
  mo2.player = player2;
  player2.viewZ = mo2.z + player2.viewHeight | 0;
  P_SetupPsprites(player2);
  P_SetLightLevel(sim);
  const totalSecret = P_SpawnLightSpecials();
  return { map: map2, sim, player: player2, subSectorAt: subSectorAt2, sectorAt: sectorAt2, exitRequested: () => exitRequested, totalKills, totalItems, totalSecret };
}
...
var level9 = G_LoadLevel(wad, `E${demo.episode}M${demo.map}`);
var { player } = level9;
var mo = player.mo;
