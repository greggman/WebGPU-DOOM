...
// meta target (rgba16uint): .rg = uv*65535, .b = type (0 for world), .a = category.
...
fn fs(in : VsOut) -> ${gb ? 'FsOut' : '@location(0) vec4f'} {
  ...
  // meta: per-surface uv (wrapped to a tile), no type, category = SID_WORLD (1).
  ${gb ? 'var o : FsOut; o.color = vec4f(rgb, 1.0); o.nd = vec4f(normal, in.viewDepth); let uvw = fract(in.uv); o.mval = vec4u(u32(uvw.x * 65535.0), u32(uvw.y * 65535.0), 0u, 1u); return o;' : 'return vec4f(rgb, 1.0);'}
}
`;
