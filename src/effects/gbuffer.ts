...
export const showNormals: PostEffect = {
  ...
  glsl: `void mainImage(out vec4 c, in vec2 fc) {
    ...
    c = vec4(normalize(normal0(uv)) * 0.5 + 0.5, 1.0);
  }`,
};
...
export const showDepth: PostEffect = {
  ...
  glsl: `void mainImage(out vec4 c, in vec2 fc) {
    float z = clamp(depth0(fc / iResolution.xy) / 1500.0, 0.0, 1.0);
...
export const outline: PostEffect = {
  ...
  glsl: `void mainImage(out vec4 c, in vec2 fc) {
    ...
    float d0 = depth0(uv);
    vec3 n0 = normalize(normal0(uv));
    vec2 o[4];
    o[0] = vec2(px.x, 0.0); o[1] = vec2(-px.x, 0.0);
    o[2] = vec2(0.0, px.y); o[3] = vec2(0.0, -px.y);
    float e = 0.0;
    for (int i = 0; i < 4; i++) {
      e += abs(depth0(uv + o[i]) - d0) / max(d0, 1.0);
      e += 1.0 - clamp(dot(normalize(normal0(uv + o[i])), n0), 0.0, 1.0);
    }
